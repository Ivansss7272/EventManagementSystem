...
// New Function: Filter events by given date range
func filterEventsByDate(w http.ResponseWriter, r *http.Request) {
    startParam := r.URL.Query().Get("start")
    endParam := r.URL.Query().Get("end")
    var filteredEvents []Event

    startDate, errStart := time.Parse(time.RFC3339, startParam)
    endDate, errEnd := time.Parse(time.RFC3339, endParam)

    if errStart != nil || errEnd != nil {
        http.Error(w, "Invalid date format. Please use RFC3339 format.", http.StatusBadRequest)
        return
    }

    events, _ := fetchUpcomingEvents()
    for _, ev := range events {
        if ev.Date.After(startDate) && ev.Date.Before(endDate) {
...
func main() {
    ...
    // Routing for the new functionality
    http.HandleFunc("/filter-events", filterEventsByDate)
